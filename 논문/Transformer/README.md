...
> √d_k로 나눠 안정화합니다.
> $Q$와 $K$의 차원($d_k$)이 커질수록, 이들의 내적값은 분산(Variance)이 매우 커집니다. 분산이 커지면 softmax 결과값이 특정 항목에만 쏠리게 되어(One-hot 형태), 역전파 시 기울기 소실(Vanishing Gradient) 문제가 발생합니다. 이를 방지하기 위해 분산을 1로 맞추는 스케일링 과정입니다.
